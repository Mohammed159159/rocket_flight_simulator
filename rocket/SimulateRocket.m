...
classdef SimulateRocket < handle
    ...
    methods
        ...
        function simulation = SimulateRocket(rocket, target_altitude_km_input, target_time_minutes_input, time_step_input)
            ...
            % You could also store initial atmospheric conditions if needed for separate plots
        ...
        function runSimulation(simulation) % 'simulation' is passed by reference (handle class behavior)
            ...
            while simulation.rocket.altitude < simulation.targetAltitude_m && ... % Check altitude target
                    ...
                    simulation.rocket.mass > 0.01 && ...                  % Avoid division by zero, ensure fuel is left
                ...
                % --- Calculate forces/acceleration for *this* step before updating state ---
                % These values will be used for history for the state *at the start* of the update
                current_net_force = simulation.rocket.netForce(); % Calculate net force
                current_acceleration = current_net_force / simulation.rocket.mass; % Calculate acceleration

                % --- Update the Rocket's State ---
                % The rocket simulationect updates its own properties (time, altitude, velocity, mass)
                ...
                simulation.simHistory.acceleration = [simulation.simHistory.acceleration; current_acceleration];
                simulation.simHistory.net_force = [simulation.simHistory.net_force; current_net_force];
        ...
        function plotResults(simulation)
            ...
            % Optional: Plot Net Force
            % figure('Name', 'Net Force Profile');
            % plot(simulation.simHistory.time, simulation.simHistory.net_force, 'c-');
            % xlabel('Time (s)');
            % ylabel('Net Force (N)');
